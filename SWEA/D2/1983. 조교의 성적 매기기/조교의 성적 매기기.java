		...
		*/
		...
		for(int test_case = 1; test_case <= T; test_case++)
		{
            ...
            for(int i = 0;i<n;i++){
                ...
                double score = (middleExam * 0.35) + (finalExam * 0.45) + (homework * 0.2);
                students[i] = score;
            }
            double objectScore = students[k-1]; // 등급을 반환하고자 하는 학생의 점수
            Arrays.sort(students);
           	
            int grade = n/10;
            String[] grades = {"D0","C-","C0","C+","B-","B0","B+","A-","A0","A+"};
            for(int i = 0;i<n;i++){
            	if(students[i] == objectScore){
                	System.out.println("#" + test_case + " " + grades[i/grade]);
                    break;
                }
            }
		}
